/**
 * Food Log Calendar Modal
 * Shows food logging status indicators:
 * - Green dot: Food logged for that day
 * - Red dot: No food logged for that day
 */
...
interface DayStatus {
  [key: string]: boolean; // true = has food, false = no food
}

const FoodLogCalendarModal = ({
  isVisible,
  onClose,
  onDateSelect,
  selectedDate: initialSelectedDate,
}: FoodLogCalendarModalProps) => {
  ...
  const loadMonthStatuses = async (date: Date) => {
    ...
    // Load all food logs for the month in parallel
    const promises = [];
    for (let day = 1; day <= daysInMonth; day++) {
      const dateToCheck = new Date(year, month, day);
      promises.push(
        loadFoodLog(dateToCheck).then((log) => {
          const key = `${year}-${month}-${day}`;
          statuses[key] = hasFoodEntries(log);
        })
      );
    }
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasFood = dayStatuses[statusKey];
...
export default FoodLogCalendarModal;
